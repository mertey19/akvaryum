...
İşletme bilgileri, iletişim değerleri, ürünler ve site metinleri yönetim panelinden (`/yonetim`, bkz. [ADMIN.md](ADMIN.md)) düzenlenir ve hemen yayına girer. Başlangıç değerleri `src/lib/content/seed.ts` dosyasındadır; `src/lib/config.ts` yalnızca demo modu, site adresi, indeksleme ve `customProductionVerified` gibi ortam ayarlarını tutar. Görünür tam marka adı `fullName`, büyük harfli marka satırı `brandLabel` ve logo `logoPath` alanlarındadır. Telefon, e-posta, adres ve saat alanlarını yalnızca işletme tarafından doğrulanınca doldurun. WhatsApp numarası panelde uluslararası biçimde, ülke koduyla girilir; boş numara WhatsApp düğmesi oluşturmaz. `.env.example` dosyasını `.env.local` olarak kopyalayın; `SITE_URL` gerçek HTTPS adresidir.

Arama motoru erişimi demo modundan ayrıdır ve kullanıcının isteğiyle 13 Eylül 2026 tarihinde açıldı. `SITE_URL` boşsa Vercel'in `VERCEL_PROJECT_PRODUCTION_URL` değeri kullanılır. `indexable` yalnızca site adresi bilindiğinde, Vercel'de production ortamında ve `ALLOW_INDEXING=false` verilmediğinde açılır; bu durumda robots izin verir, sitemap sayfa ve ürün adreslerini listeler, sayfalar `index,follow` olur. Kullanıcının isteğiyle robots hiçbir yolu engellemez; katalog, kategori, 45°/90°, teklif, konsept galerisi, favoriler ve karşılaştırma sayfaları da indekslenebilir. Katalog sayfalarının canonical adresi yalnızca kategori ve 45°/90° seçeneğini içerir; arama, sıralama ve fiyat filtresi parametreleri bu adrese bağlanır. Yerel geliştirmede ve preview dağıtımlarında site `noindex` kalır.

Google'da görünen site adı kullanıcı isteğiyle 15 Eylül 2026 tarihinde `DSN Akvaryum` olarak ayarlandı. Ana sayfa başlığı yalnızca `name` değeridir, diğer sayfalar `Başlık | DSN Akvaryum` biçimini kullanır. Ana sayfada `WebSite` yapılandırılmış verisi (`name`, `alternateName`), tüm sayfalarda `og:site_name` ve Organization verisinde `alternateName` bulunur. Ana sayfa açıklaması slogan yerine işletme, konum ve ürün bilgisini içerir; sayfadaki hero sloganı değişmedi. Site adı ve site bağlantıları (sitelinks) Google tarafından otomatik belirlenir, garanti edilemez.
